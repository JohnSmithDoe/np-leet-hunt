...
- Lefthook (`lefthook.yml`, installed via the `prepare` script) runs a parallel pre-commit: `eslint --fix` (js/ts/html), `stylelint --fix` (css/scss), and `prettier --write` (json/md/yaml) on staged files, re-staging fixes.
