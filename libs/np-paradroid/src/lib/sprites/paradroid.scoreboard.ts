import { DUEL_LABEL_BASE, PALETTE } from '@shared/np-config';
...
export class ParadroidScoreboard {
    ...
    place(bounds: Phaser.Geom.Rectangle): void {
        ...
        this.#you = this.#label('YOU', bounds.left + bounds.width * 0.06, y, PALETTE.win, labelSize).setOrigin(0, 1);
        this.#droid = this.#label('DROID', bounds.right - bounds.width * 0.06, y, PALETTE.lose, labelSize).setOrigin(
            1,
            1
        );
        this.#score = this.#label('0 — 0', bounds.centerX, y, PALETTE.cyan, scoreSize).setOrigin(0.5, 1);
    }

    #label(text: string, x: number, y: number, color: string, fontSizePx: number): Phaser.GameObjects.Text {
        return this.#scene.add.text(x, y, text, { ...DUEL_LABEL_BASE, color, fontSize: `${fontSizePx}px` });
    }
