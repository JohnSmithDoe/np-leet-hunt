...
export class NPSpaceMap extends NPGameObjectList<NPGameObject> {
    #debug = false;
    init = () => {
        ...
        if (this.#debug) this.#debugDraw(map);
        const allPlanets = [...map.coords.planets, ...map.coords.outerSpace];
        for (const coords of allPlanets) {
            const connections = map.coords.outerSpace.includes(coords) ? 2 : 3;
            const closest = getClosest(coords, allPlanets, connections);
